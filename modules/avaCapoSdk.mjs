...
class AvaCapoTalkingHead {
    ...
    if (r.words) {
      ...
      for (let i = 0; i < r.words.length; i++) {
        ...
        if (word.length) {
          ...
          const dTotal =
            r.vtimes[r.visemes.length - 1] + r.vdurations[r.visemes.length - 1];
          const overdrive = Math.min(
            duration,
            Math.max(0, duration - r.visemes.length * 150)
          );
          let level =
            0.2 + this.convertRange(overdrive, [0, duration], [0, 0.4]);
          duration = Math.min(duration, r.visemes.length * 200);
          if (dTotal > 0) {
            for (let j = 0; j < r.visemes.length; j++) {
              const t = time + (r.vtimes[j] / dTotal) * duration;
              const d = (r.vdurations[j] / dTotal) * duration;
              lipsyncAnim.push({
                template: { name: "viseme" },
                ts: [
                  t - Math.min(60, (2 * d) / 3),
  ...
  async speakResponses(json, arraybuffer) {
    if (!json || !arraybuffer) return;
  
    try {
      // Decode the audio buffer
      const audiobuffer = await this.audioCtx.decodeAudioData(arraybuffer);
  
      // Build the ttsAudio object
      const ttsAudio = {
        audio: audiobuffer,
        words: [],
        wtimes: [],
      ...
      if (json.list_of_Words) {
        json.list_of_Words.forEach(x => {
          ttsAudio.words.push(x.word);
  
          let startTime = 1000 * x.start;
          if (startTime > 150) startTime -= 150;
          ttsAudio.wtimes.push(startTime);
  
          let duration = 1000 * (x.end - x.start);
          if (duration > 20) duration -= 20;
          ttsAudio.wdurations.push(duration);
        });
      }
  
      // Process visemes
      if (json.visemes) {
        for (let key in json.visemes) {
          const visemeData = json.visemes[key];
          visemeData.visemes.forEach((viseme, i) => {
            ttsAudio.visemes.push(viseme);
            ttsAudio.vtimes.push(visemeData.times[i]);
            ttsAudio.vdurations.push(visemeData.durations[i]);
          });
        }
      }
      ...
      json.list_of_Words.forEach((x, index) => {
        if (index > 0 && x.start > 2 && x.word.length > 10) {
          ttsAudio.markers.push(startSegment);
          let markerTime = Math.max(0, 1000 * x.start - 1000);
          ttsAudio.mtimes.push(markerTime);
        }
      });
  
      // Call the head's speakAudio function with the processed ttsAudio object
      this.speakAudio(ttsAudio);
  
    } catch (error) {
      console.log('Error in speakAudio:', error);
    }
  }
